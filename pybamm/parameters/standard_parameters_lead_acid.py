...
U_Ox_dim = pybamm.Parameter("Oxygen reference OCP vs SHE")
j0_S_n_ref_dimensional = pybamm.Parameter(
    "Negative electrode reference exchange-current density"
)
j0_S_p_ref_dimensional = pybamm.Parameter(
    "Positive electrode reference exchange-current density"
)
j0_Ox_n_ref_dimensional = pybamm.Parameter(
    "Negative electrode reference exchange-current density (oxygen)"
)
j0_Ox_p_ref_dimensional = pybamm.Parameter(
    "Positive electrode reference exchange-current density (oxygen)"
)
j0_Hy_n_ref_dimensional = pybamm.Parameter(
    "Negative electrode reference exchange-current density (hydrogen)"
)
j0_Hy_p_ref_dimensional = pybamm.Parameter(
    "Positive electrode reference exchange-current density (hydrogen)"
)
s_plus_S_n_dim = pybamm.Parameter("Negative electrode cation signed stoichiometry")
s_plus_S_p_dim = pybamm.Parameter("Positive electrode cation signed stoichiometry")
ne_S_n = pybamm.Parameter("Negative electrode electrons in reaction")
ne_S_p = pybamm.Parameter("Positive electrode electrons in reaction")
ne_n = ne_S_n
ne_p = ne_S_p
...
c_ox_ref_dim = pybamm.Parameter("Reference oxygen molecule concentration")
...
# Reaction timescales
# should this be * F?
tau_r_n = 1 / (j0_S_n_ref_dimensional * a_n_dimensional * c_e_typ ** 0.5)
tau_r_p = 1 / (j0_S_p_ref_dimensional * a_p_dimensional * c_e_typ ** 0.5)
...
# Electrochemical reactions
U_Ox_n = (U_Ox_dim - U_n_ref) / potential_scale
U_Ox_p = (U_Ox_dim - U_p_ref) / potential_scale
s_plus_S_n = s_plus_S_n_dim / ne_S_n
s_plus_S_p = s_plus_S_p_dim / ne_S_p
...
s_n = -(s_plus_S_n + t_plus)  # Dimensionless rection rate (neg)
s_p = -(s_plus_S_p + t_plus)  # Dimensionless rection rate (pos)
s = pybamm.Concatenation(
    pybamm.Broadcast(s_n, ["negative electrode"]),
    pybamm.Broadcast(0, ["separator"]),
    pybamm.Broadcast(s_p, ["positive electrode"]),
)
j0_S_n_ref = j0_S_n_ref_dimensional / interfacial_current_scale_n
j0_S_p_ref = j0_S_p_ref_dimensional / interfacial_current_scale_p
m_n = j0_S_n_ref
m_p = j0_S_p_ref
j0_Ox_n_ref = j0_Ox_n_ref_dimensional / interfacial_current_scale_n
j0_Ox_p_ref = j0_Ox_p_ref_dimensional / interfacial_current_scale_p
j0_Hy_n_ref = j0_Hy_n_ref_dimensional / interfacial_current_scale_n
j0_Hy_p_ref = j0_Hy_p_ref_dimensional / interfacial_current_scale_p
c_ox_ref = c_ox_ref_dim / c_e_typ
C_dl_n = (
...
# Surface area change
xi_n = pybamm.Parameter("Negative electrode surface area parameter")
xi_p = pybamm.Parameter("Positive electrode surface area parameter")

# Electrolyte properties
beta_surf_n = -c_e_typ * DeltaVsurf_n / ne_S_n  # Molar volume change (lead)
beta_surf_p = -c_e_typ * DeltaVsurf_p / ne_S_p  # Molar volume change (lead dioxide)
...
beta_liq_n = -c_e_typ * DeltaVliq_n / ne_S_n  # Molar volume change (electrolyte, neg)
beta_liq_p = -c_e_typ * DeltaVliq_p / ne_S_p  # Molar volume change (electrolyte, pos)
...
def chi(c_e):
    return (
        ...
        * 2
        * (1 - t_plus)
        / (V_w * c_T(c_e_typ * c_e, c_e_typ * c_ox_ref, 0))
    )
